...
`ctx.agentPresets` owns discovery, authoring, and mounting. You reach it by defining a temporary Host Plugin with `cordis_define` and activating it with `cordis_run` — a Probe whose `apply` reads the service with `ctx.get('agentPresets')` and registers a model tool through `harness`. `cordis_run` returns only an activation acknowledgement, so the registered tool is how a service answer gets back to you, and it becomes callable on your next model step.

Read the current signatures with `cordis_inspect_query` first — provider `Service`, method `listService`, input `{ "service": "agentPresets" }`. What this skill relies on:
...
- `remove(id)` — delete a user-authored preset.
- `standingKeyFor(id?)` — mount-validate one preset (see below); `undefined` means the default id.
...
return {
  apply(ctx) {
    ...
    harness.registerTool(ctx, harness.defineTool({
      ...
      async execute(args) {
        try {
          await presets.standingKeyFor(args.id)
          return 'mounted OK: ' + args.id
        } catch (error) {
...
A host-only Probe needs no Client approval. Undefine it with `cordis_undefine` when you are done; it is a probe, not a capability to leave behind.
...
Whether a row publishes a service is not visible from its name, and package READMEs are absent from an installed deployment. The `Service` Inspect provider (`listService`) lists every service key current in the process, but not which fiber owns each one — ownership is what matters, and it is surfaced only by action: for a row not in your current composition, mount-validate and read the rejection, which names the offending service.
...
The Inspect providers report THIS session's runtime, so they confirm what a row does in the composition you are already in, never what your new preset will do.
...
A dynamic Plugin runs JavaScript against the live runtime in this process only and disappears on restart. It is for probing, not for shipping a capability: a capability belongs in a composition file.
